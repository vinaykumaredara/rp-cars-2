...
const Modal: React.FC<ModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleGoogleSignIn = async () => {
    ...
    if (error) {
      ...
      // To bypass restrictive iframe sandbox policies, we create a temporary anchor tag
      // with `target="_top"` and programmatically click it. This robustly instructs the
      // browser to navigate the top-level frame, resolving the "permission to navigate" error.
      ...
      setError('Could not retrieve Google sign-in URL. Please try again.');
      setIsGoogleLoading(false);
    }
    // If the redirect succeeds, the page navigates away, so we don't need to set loading to false.
  };
  ...
  return (
    ...
    >
      <div 
        ref={modalRef}
        tabIndex={-1}
        className="bg-white rounded-lg shadow-2xl w-full max-w-md mx-auto p-8 transform transition-all duration-300 ease-in-out focus:outline-none"
        onClick={(e) => e.stopPropagation()}
        style={{ animation: 'fadeInScale 0.3s forwards' }}
      >
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold text-foreground">{isSignUp ? 'Create Account' : 'Sign In'}</h2>
        ...
        <form onSubmit={handleAuthAction}>
          ...
          <button type="submit" disabled={loading || isGoogleLoading} className="w-full py-3 rounded-lg bg-primary text-white font-semibold hover:bg-primary-hover transition-all duration-300 shadow-sm hover:shadow-md mb-4 disabled:bg-opacity-50">
        ...
        <button type="button" onClick={handleGoogleSignIn} disabled={loading || isGoogleLoading} className="w-full py-3 flex items-center justify-center rounded-lg border border-gray-300 text-foreground font-medium hover:bg-gray-50 transition-all duration-300 disabled:bg-gray-200">
